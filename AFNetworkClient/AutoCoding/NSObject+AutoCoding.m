...
+ (instancetype)ac_objectWithDictionary:(NSDictionary *)dictionary
{
    ...
    {
        BOOL bReturnHasUndderLine=NO;
#ifdef SERVER_KEY_UNDERLINE
        bReturnHasUndderLine = YES;
#endif
        id value;
        
        if (bReturnHasUndderLine) {
            id propertyName = [self mj_underlineFromCamel:property];
...
- (NSString *)mj_underlineFromCamel:(NSString*)underLineString
{
    if (underLineString.length == 0) return underLineString;
    NSMutableString *string = [NSMutableString string];
    for (NSUInteger i = 0; i<underLineString.length; i++) {
        unichar c = [underLineString characterAtIndex:i];
